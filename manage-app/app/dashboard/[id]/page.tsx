...
export default function Visual() {
  ...
  if (!chartMeta) return <p className="flex self-center justify-self-center m-50">Chart not found</p>;
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 m-10">
      ...
      <div className="flex gap-4 mb-4">
        <button className="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600" onClick={() => setShowEdit(true)}>Edit</button>
      </div>
      ...
      {showEdit && (
        <EditGraph id={chartMeta.name} chartMeta={chartMeta} onClose={() => setShowEdit(false)} />
      )}
    </div>
  );
}
