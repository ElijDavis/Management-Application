...
const EditGraph = ({name, chartMeta, onClose}: EditGraphProps) => {
  ...
  const [colors, setColors] = useState<string[]>(chartMeta.options?.colors || chartMeta.yKeys.map(() => "#3b82f6"));
  const [rangeStart, setRangeStart] = useState(chartMeta.options?.visibleRange?.start ?? 0);
  const [rangeEnd, setRangeEnd] = useState(chartMeta.options?.visibleRange?.end ?? (typeof chartMeta.source !== "string" ? chartMeta.source.labels.length : 0));

  const [showLegend, setShowLegend] = useState(chartMeta.options?.showLegend ?? true);
  const [showToast, setShowToast] = useState(false);

  const handleSave = async () => {
    const updatedOptions = { xAxisTitle, yAxisTitle, colors, visibleRange: { start: rangeStart, end: rangeEnd }, showLegend };
    try {
  ...
  return(
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-black/30 bg-opacity-50 z-50" onClick={onClose}>
      <div className="bg-foreground rounded-lg flex flex-col justify-center items-center text-background w-2/3 h-2/3" onClick={(e) => e.stopPropagation()}>
        <h1 className="text-background text-2xl mb-10">Edit Chart - Coming Soon!</h1>
      ...
      {showToast && <Toast message="Chart updated successfully!" onClose={() => setShowToast(false)} />}
    </div>
  )
}
